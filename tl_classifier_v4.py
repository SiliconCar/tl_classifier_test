...
if __name__ == '__main__':
        ...
        #TEST_IMAGE_PATHS= glob(os.path.join('img_no_light/', '*.jpg'))
        TEST_IMAGE_PATHS= glob(os.path.join('traffic_light_images/', '*.jpg'))
        for i, image_path in enumerate(TEST_IMAGE_PATHS):
            ...
            #img_full = img_full.crop((0,100, 600, 450))
            ...
            if np.array_equal(b, np.zeros(4)):
               ...
               #filename='green1_real_'+str(i)+'.jpg'
               #cv2.imwrite(filename,cv2.cvtColor(img_full_np_copy[b[0]:b[2], b[1]:b[3]], cv2.COLOR_RGB2BGR))
               cv2.rectangle(img_full_np,(b[1],b[0]),(b[3],b[2]),(0,255,0),3)
               plt.imshow(img_full_np)
               plt.show()
